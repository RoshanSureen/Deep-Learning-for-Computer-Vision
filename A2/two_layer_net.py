...
def nn_forward_pass(params, X):
    ...
    hidden1 = torch.mm(X,W1) + b1
    hidden = torch.max(torch.zeros_like(hidden1), hidden1)
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    # numeric stability shift values of scores so that the highest number is 0
    ...
    idxs = [torch.arange(N), y]
    correct_logprobs = -torch.log(probs[idxs])
    ...
    dh1 = torch.mm(dscores, W2.t()) # dL/dh1 = dscores/dh1 * dL/dscores
    dW2 = torch.mm(h1.t(), dscores) # dL/dW2 = dscores/dW2 * dL/dscores
    db2 = torch.sum(dscores, dim=0) # dL/db2 = dscores/db2 * dL/dscores

    # backprop the ReLU non-linearity (set -ve values to 0)
    dh1[h1 <= 0] = 0

    # finally backpropogate into W1,b1
    # h1 = max(0, X*W1 + b1)
    dW1 = torch.mm(X.t(),dh1) # dL/dW1 = dh1/dW1 * dL/dh1
    db1 = torch.sum(dh1, dim=0) # dL/db1 = dh1/db1 * dL/dh1
...
def nn_predict(params, loss_func, X):
  ...
  hidden1 = torch.mm(X, params['W1']) + params['b1']
  hidden_layer = torch.max(torch.zeros_like(hidden1), hidden1)
  scores = torch.mm(hidden_layer, params['W2']) + params['b2']
...
def find_best_net(data_dict, get_param_set_fn):
  ...
  results = get_param_set_fn()
  learning_rates, hidden_sizes = results[0], results[1]
  regularization_strengths, learning_rate_decays = results[2], results[3] 

  for lr in learning_rates:
    for hs in hidden_sizes:
      for reg in regularization_strengths:
        for ni in num_iters:
          cand_net_model = TwoLayerNet(3 * 32 * 32, hs, 10, 
                                      device=data_dict['X_train'].device, 
                                      dtype=data_dict['X_train'].dtype)

          cand_stats = cand_net_model.train(data_dict['X_train'], data_dict['y_train'], 
                                            data_dict['X_val'], data_dict['y_val'],
                                            num_iters=ni, batch_size=1000,
                                            learning_rate=lr, learning_rate_decay=0.95,
                                            reg=reg, verbose=False)
          
          y_train_pred = cand_net_model.predict(data_dict['X_train'])
          train_acc = torch.mean((y_train_pred == data_dict['y_train']).float())
          
          y_val_pred = cand_net_model.predict(data_dict['X_val'])
          cand_val_acc = torch.mean((y_val_pred == data_dict['y_val']).float())

          print('lr = {0:2.2e}'.format(lr), ' hs = {0:2.2e}'.format(hs), 
                'reg = {0:2.2e}'.format(reg), ' ni = {0:2.2e}'.format(ni),
                'train_acc = {0:2.4e}'.format(train_acc),
                'val_acc = {0:2.4e}'.format(cand_val_acc))
          
          if cand_val_acc > best_val_acc:
            best_val_acc = cand_val_acc
            best_net = cand_net_model
            best_stat = cand_stats
