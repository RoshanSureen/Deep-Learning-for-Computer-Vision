...
def compute_distances_no_loops(x_train, x_test):
  ...
  norm1 = torch.sum(x_train_flat.pow(2), dim=1).view(-1,1)
  norm2 = torch.sum(x_test_flat.pow(2), dim=1).view(-1,1)

  dists = (norm1 + norm2.t()) - 2.0 * (torch.mm(x_train_flat, x_test_flat.t()))
...
def predict_labels(dists, y_train, k=1):
  ...
  #   test_sample = dists[:, i]
  #   _, idxs = torch.topk(test_sample,k = k, largest= False)    
  #   labels = y_train[idxs]
  #   vals, counts = torch.unique(labels, return_counts = True)
  #   mask = (counts == torch.max(counts))
  #   y_pred[i] = torch.min(vals[mask])

  _, idxs = torch.topk(dists, k=k, dim=0, largest=False)
  labels = y_train[idxs]
  vals, _ = torch.mode(labels, dim=0)
  y_pred = vals
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
      temp = list(range(num_folds))
      temp.pop(i)

      validation_set_x = x_train_folds[i]
      training_set_x = torch.cat([x_train_folds[j] for j in temp])

      validation_set_y = y_train_folds[i]
      training_set_y = torch.cat([y_train_folds[j] for j in temp])

      classifier = KnnClassifier(training_set_x, training_set_y)
      acc = classifier.check_accuracy(validation_set_x, validation_set_y, k = k, quiet = True)
...
def knn_get_best_k(k_to_accuracies):
  ...
  for k, acc_lists in k_to_accuracies.items():
    curr_mean_acc = torch.sum(torch.tensor(acc_lists)) / len(acc_lists)
    if curr_mean_acc > mean_acc:
      mean_acc = curr_mean_acc
      best_k = k
