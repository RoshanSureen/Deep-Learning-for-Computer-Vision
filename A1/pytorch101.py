...
def mutate_tensor(x, indices, values):
  ...
  for ix, indexes in enumerate(indices):
    x[indexes] = values[ix]
...
def count_tensor_elements(x):
  ...
  num_elements = reduce(lambda x, y: x*y, list(x.shape))
...
def multiples_of_ten(start, stop):
  ...
  if stop > 9:
    if (start + (10 - start % 10)) <= stop:
      x = torch.arange(start + (10 - start % 10),stop+1, 10, dtype=torch.float64)
    elif ((start % 10) == 0) or ((start == stop) and ((start % 10) == 0)):
      x = torch.tensor([start], dtype=torch.float64)    
    else:
...
def reshape_practice(x):
  ...
  x1 = x.view(2,3,4)
  y = x1.transpose(0,1).reshape(3,8)
...
def normalize_columns(x):
  ...
  y = x.clone()
  mean_y = torch.sum(y, dim=0) / y.shape[0]
  std_y = torch.sqrt(((y - mean_y) **2).sum(dim = 0) / (y.shape[0] - 1))
